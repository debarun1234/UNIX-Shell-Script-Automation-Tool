...
def check_fully_commented_structures(lines, sections, functions, loops, keywords):
    ...
    fully_commented_functions = set()
    fully_commented_loops = set()
    ...
    for function in functions:
        ...
        if has_meaningful_content and all_meaningful_content_commented:
            fully_commented_functions.add(function['name'])
    ...
    for loop in loops:
        ...
        if has_keyword_content and all_keyword_content_commented:
            fully_commented_loops.add(f"{loop['type']}_line_{loop['start'] + 1}")
    ...
    return fully_commented_sections, fully_commented_functions, fully_commented_loops

def renumber_sections(lines, sections, fully_commented_sections):
...
def generate_changelog(stats, sections, functions, fully_commented_sections, 
                      fully_commented_functions, renumber_map, global_replacements):
    ...
    if not stats['modified_sections'] and not stats['modified_functions']:
        ...
        if fully_commented_sections:
            changelog.append("# SECTIONS FULLY COMMENTED OUT:")
            for sec_num in sorted(fully_commented_sections):
                section = next(s for s in sections if s['num'] == sec_num)
                changelog.append(f"#   • Section {sec_num}: {section['description']}")
            changelog.append("#")
        
        # Fully commented functions
        if fully_commented_functions:
            changelog.append("# FUNCTIONS FULLY COMMENTED OUT:")
            for func_name in sorted(fully_commented_functions):
                changelog.append(f"#   • Function: {func_name}")
            changelog.append("#")
...
def insert_changelog(lines, changelog):
    ...
    marker = "# CHANGELOG SUMMARY WILL BE CREATED HERE below"
    separator = "###############################################################"
...
def fix_indentation(lines):
    ...
    for i, line in enumerate(lines):
        ...
        if (re.match(r'^\s*if\s+', stripped) or 
            ...
            re.match(r'^\s*for\s+', stripped) or
            re.match(r'^\s*while\s+', stripped) or
            re.match(r'^\s*until\s+', stripped) or
...
def process_script(script_path, keywords_path, output_path, verbose=False):
    ...
    if verbose:
        print(f"Found {len(sections)} sections, {len(functions)} functions, and {len(loops)} loops")
        for section in sections:
    ...
    new_lines, stats = process_keyword_matching(new_lines, keywords, sections, functions, loops, orig_commented)    # Check for fully commented structures
    fully_commented_sections, fully_commented_functions, fully_commented_loops = check_fully_commented_structures(
        new_lines, sections, functions, loops, keywords)
    
    # Renumber active sections
    renumber_map = renumber_sections(new_lines, sections, fully_commented_sections)
    
    # Apply global replacements
    global_replacements = apply_global_replacements(new_lines)
      # Generate and insert changelog
    changelog = generate_changelog(stats, sections, functions, fully_commented_sections,
                                 fully_commented_functions, renumber_map, global_replacements)
    ...
    try:
        ...
        print(f"- Functions fully commented: {len(fully_commented_functions)}")
